-- ============================================================================
-- Pausa global de puntos (un solo flag, en app_settings.points_paused).
-- Cuando está en 'true', las 4 funciones que otorgan puntos devuelven sin
-- tocar nada (ni total_points, ni point_events, ni contadores de racha/ad) —
-- reversible al 100%, no pierde ni corrompe ningún estado mientras dura.
-- Pensado para congelar el ranking justo antes/durante el sorteo de premios.
-- ============================================================================

INSERT INTO app_settings (key, value) VALUES ('points_paused', 'false')
...
DECLARE
  ...
  v_paused  boolean;
BEGIN
  SELECT (value = 'true') INTO v_paused FROM public.app_settings WHERE key = 'points_paused';
  ...
  IF COALESCE(v_paused, false) THEN
    RETURN v_current; -- pausado: no toca nada, devuelve el total sin cambios
  END IF;
...
BEGIN
  SELECT (value = 'true') INTO v_paused FROM public.app_settings WHERE key = 'points_paused';
  IF COALESCE(v_paused, false) THEN
    RETURN jsonb_build_object('awarded', false, 'paused', true);
  END IF;
